...
async function initializeGatewayV2() {
    ...
    try {
        ...
        const privateKeyBytes = (0, utils_1.fromB64)(privateKeyB64);
        ...
        if (privateKeyBytes.length === 33) {
            ...
            if (schemeFlag === 0x00) {
                // Ed25519
                keypair = ed25519_1.Ed25519Keypair.fromSecretKey(actualKey);
            }
            else if (schemeFlag === 0x01) {
                // Secp256k1
                keypair = secp256k1_1.Secp256k1Keypair.fromSecretKey(actualKey);
            }
            else {
    ...
    const gatewayContent = gatewayState.data?.content;
    ...
    const tokenContent = tokenState.data?.content;
    ...
    try {
        ...
        if (dryRun.effects.status.status === 'success') {
            ...
            if (dryRun.effects.created && dryRun.effects.created.length > 0) {
                ...
                dryRun.effects.created.forEach((obj) => {
    ...
    try {
        ...
        if (result.effects?.created && result.effects.created.length > 0) {
            ...
            result.effects.created.forEach((obj) => {
                ...
                if (result.objectChanges) {
                    const change = result.objectChanges.find((c) => c.objectId === obj.reference.objectId);
                    if (change && 'objectType' in change && change.objectType?.includes('GatewayCapabilities')) {
        ...
        const updatedContent = updatedGateway.data?.content;
        ...
        if (error.message?.includes('E_ALREADY_MINTER')) {
