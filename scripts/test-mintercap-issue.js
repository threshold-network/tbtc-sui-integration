...
async function demonstrateMinterCapIssue() {
    ...
    try {
        const client = new client_1.SuiClient({ url: TESTNET_CONFIG.rpc });
        ...
        if (tokenState.data?.content?.dataType === 'moveObject') {
            const fields = tokenState.data.content.fields;
        ...
        const minterCap = gatewayOwnedObjects.data.find(obj => obj.data?.type?.includes('MinterCap'));
        if (minterCap) {
...
async function checkMinterCapStatus() {
    ...
    try {
        ...
        const minterAddedTx = transactions.data.find(tx => tx.events?.some(event => event.type.includes('MinterAdded')));
        if (minterAddedTx) {
            ...
            const minterCap = createdObjects?.find(obj => obj.reference.objectId && obj.owner);
            if (minterCap && typeof minterCap.owner === 'object' && 'AddressOwner' in minterCap.owner) {
                ...
                console.log('- Owner:', minterCap.owner.AddressOwner);
                console.log('- Is Gateway:', minterCap.owner.AddressOwner === TESTNET_CONFIG.objects.gatewayState);
