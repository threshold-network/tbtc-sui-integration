...
async function initializeGateway() {
    ...
    try {
        const client = new SuiClient({ url: TESTNET_V2_CONFIG.rpc });
        ...
        if (result.objectChanges) {
            const gatewayCapabilitiesCreated = result.objectChanges.find(
        ...
        if (updatedGateway.data?.content?.dataType === 'moveObject') {
            const fields = updatedGateway.data.content.fields as any;
            console.log('   - Is Initialized:', fields.is_initialized);
